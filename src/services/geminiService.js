...
const convertToBase64 = async (imageInput) => {
  ...
  if (imageInput?.data) {
    return { data: imageInput.data, mimeType: imageInput.mimeType || "image/jpeg" };
  }
...
Return this exact structure:
{
  ...
  "upcycleIdeas": ["Creative idea 1", "Creative idea 2", "Creative idea 3"]
      ...
      if (jsonMatch) {
        ...
        return {
          ...
          recommendation: analysis.recommendation || "Please recycle responsibly.",
  ...
  async getWasteReductionTips(category) {
    try {
      ...
      return { success: true, tips: Array.isArray(tips) ? tips : this._fallbackTips(category).tips };
    } catch {
