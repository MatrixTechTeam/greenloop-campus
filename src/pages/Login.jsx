...
const Login = () => {
  ...
  // Handle redirect result on mount (for mobile redirect flow)
  React.useEffect(() => {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (isMobile) {
        // Use redirect on mobile — no navigate needed, page redirects automatically
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 relative overflow-hidden bg-gradient-to-br from-primary-50/50 via-white to-sage-50/30">
      ...
      <div className="absolute inset-0 overflow-hidden">
        ...
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[500px] bg-primary-50 rounded-full blur-3xl opacity-20"></div>
      </div>
          ...
          <div className="text-center mb-8">
            <div className="flex justify-center mb-4">
              <div className="relative">
                ...
                <div className="relative w-16 h-16 bg-gradient-to-br from-primary-500 to-primary-700 rounded-2xl flex items-center justify-center shadow-lg">
                  <Leaf className="w-8 h-8 text-white" />
                </div>
              </div>
            </div>
            <h1 className="text-3xl font-bold text-gray-900 mb-2">GreenLoop Campus</h1>
            <p className="text-gray-500">AI-Powered Sustainability Platform</p>
          </div>
          ...
          {resetMode ? (
            <>
              ...
              <form onSubmit={handleResetPassword} className="space-y-6">
                ...
                <button type="button" onClick={() => setResetMode(false)} className="w-full text-center text-primary-600 hover:text-primary-700 font-medium transition-colors">
                  Back to Login
                </button>
              </form>
            </>
            ...
            <>
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <button type="button" onClick={() => setResetMode(true)} className="text-sm text-primary-600 hover:text-primary-700 font-medium transition-colors">
                    Forgot Password?
                  </button>
                </div>

                <button type="submit" disabled={loading} className="btn-primary w-full flex items-center justify-center gap-2">
                  {loading ? <div className="spinner-sm"></div> : <><LogIn size={18} />Sign In</>}
                </button>
              </form>
              ...
              >
                {loading ? <div className="spinner-sm"></div> : <><GoogleIcon /><span className="text-gray-700 font-medium">Sign in with Google</span></>}
              </button>
